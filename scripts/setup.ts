...
// Function to prompt for Github client credentials
async function promptForGithubClientCredentials() {
  ...
  if (!fs.existsSync(devVarsPath)) {
    ...
    try {
      ...
      console.log(
        "\x1b[33m.dev.vars file created with Github Client ID and Client Secret.\x1b[0m"
      );
    } catch (error) {
  ...
  outro(".dev.vars updated with Github Client ID and Client Secret.");
}
...
async function main() {
  try {
    ...
    await promptForGithubClientCredentials();
